let selectTab = (number) => {
  ...
  window.selectedTabIndex = number;
}
    ...
    chrome.runtime.onConnect.addListener(function (port) {
      if (port.name === "ActiveTabsConnection") {
        port.onMessage.addListener(function (msg) {
          window.activeTabsCount = msg.tabs.length;
...
function sortTabs(head, type) {
  let prevTabs = tabsList, prevTabsArray, tabsListArray, loopFinished;
    setTimeout(function() {
      ...
      head++;
      if (head < tabsList.length) sortTabs(head, type);
      loopFinished = true;
