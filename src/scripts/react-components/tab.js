...
export default class Tab extends React.Component {
  ...
  pinTab(id, pinned) {
    ...
    pinned
      ? packagedAndBroadcast(sender, 'background', 'unpinTab', id)
      : packagedAndBroadcast(sender, 'background', 'pinTab', id);
    this.setState({ pinned: !pinned });
  }
  muteTab(id, audible) {
    audible
      ? packagedAndBroadcast(sender, 'background', 'muteTab', id)
      : packagedAndBroadcast(sender, 'background', 'unmuteTab', id);
    ...
    return (
      <li key={this.props.id} data-id={this.props.id} className="tab-item">
        ...
        <ul className=" tab-actions" role="group" aria-label="options">
          ...
          <li
            title="Un/Mute Tab"
            className={`clickable` + (this.state.audible ? ` active` : ` disabled`)}
            onClick={this.muteTab.bind(this, this.props.id, this.state.audible)}
            aria-hidden="true"
          >
