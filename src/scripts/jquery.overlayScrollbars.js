    ...
    var PLUGIN = (function (framework, compatibility, instances) {
            ...
            var checkObjectProps = function (
              ...
              for (var prop in template) {
                if (
                  ...
                  var templateTypes =
                    framework.type(templateValue) != TYPES.a
                      ? [templateValue]
                      : templateValue
      ...
      function OverlayScrollbarsInstance(
        ...
        function textareaOnKeyDown(event) {
          ...
          if (textareaIsRestrictedKeyCode(keyCode)) return
        ...
        function textareaOnKeyUp(event) {
          ...
          if (index > -1) _textareaKeyDownKeyCodesList.splice(index, 1)
          if (_textareaKeyDownKeyCodesList.length === 0) {
            textareaUpdate()
            _base.update(_strAuto)
        ...
        function scrollbarCornerOnResize(event) {
          ...
          if (_resizeHorizontal || _resizeBoth)
            hostElementCSS[_strWidth] =
              _resizeDragStartSize.w + pageOffset.x - _resizeDragStartPosition.x
          if (_resizeVertical || _resizeBoth)
            hostElementCSS[_strHeight] =
              _resizeDragStartSize.h + pageOffset.y - _resizeDragStartPosition.y
        ...
        function scrollbarCornerOnResized(event) {
          ...
          if (eventIsTrusted) {
            if (_resizeReconnectMutationObserver) mutationObserversConnect()
            _base.update(_strAuto)
          }
        ...
        function bodyMinSizeChanged() {
          ...
          if (_isBody && _contentArrangeElement) {
            bodyMinSize.w = parseIntToZeroOrNumber(
              _contentArrangeElement.css(_strMinMinus + _strWidth)
            )
            bodyMinSize.h = parseIntToZeroOrNumber(
        ...
        function updateAutoContentSizeChanged() {
          ...
          var textareaValueLength =
            _isTextarea && _widthAutoCache && !_textareaAutoWrappingCache
              ...
              : 0
          var setCSS =
            !_mutationObserverConnected && _widthAutoCache && !_isTextarea
        ...
        function meaningfulAttrsChanged() {
          if (_isSleeping || _mutationObserverConnected) return false
        ...
        function textareaUpdate() {
          ...
          var minWidth =
            _viewportSize.w -
            (!_isBorderBox && !_paddingAbsoluteCache && _widthAutoCache
              ? _paddingY + _borderY
              : 0)
          var minHeight =
            _viewportSize.h -
            (!_isBorderBox && !_paddingAbsoluteCache && _heightAutoCache
              ? _paddingY + _borderY
              : 0)
          var css = {}
          ...
          width = doMeasure
            ...
            : 1
          width += _widthAutoCache
            ? _marginX +
              (!_isBorderBox ? (wrapAttrOff ? 0 : _paddingX + _borderX) : 0)
            : 0
        ...
        function update(
          ...
          //detect height auto:
          var heightAuto =
            _sizeAutoObserverAdded && !displayIsHidden
              ? Math.round(
          ...
          if (
            ...
            var contentGlueSize = {
              w:
                ...
                (textareaDynWidth
                  ? _isTextarea && widthAuto
                    ? _marginX + (!_isBorderBox ? _paddingX + _borderX : 0)
                    : 0
                  : 0),
              h: Math.max(
            ...
            if (sizeAutoCapable) {
              ...
              var maxHeight =
                ...
                (_isBorderBox ? _borderY : -_paddingX)
              ...
              if (
                ...
                if (_isTextarea)
                  textareaCoverCSS[_strHeight] =
                    parseIntToZeroOrNumber(
                      _textareaCoverElement.css(_strHeight)
                    ) - 1
            ...
            if (contentBCRect.width) {
              ...
              contentScrollSize.w +=
                parseIntToZeroOrNumber(contentBCRectW + contentBCRectMargin) -
                contentBCRectW
              contentScrollSize.h +=
                parseIntToZeroOrNumber(contentBCRectH + contentBCRectMargin) -
                contentBCRectH
            }
            ...
            var overflowAmount = {
              x: Math.max(
                0,
                Math.round(
                  (contentScrollSize.w - hostSize.w + padding.ax) * 100
                ) / 100
              ),
              y: Math.max(
                0,
                Math.round(
                  (contentScrollSize.h - hostSize.h + padding.ay) * 100
                ) / 100
              )
            }
            ...
            if (
              ...
              var addMouseTouchEvents = function (move) {
                if (_supportPassiveEvents) {
                  ...
                  if (move)
                    ...
                    _hostElement
                      .on(_strMouseTouchEnter, hostOnMouseEnter)
              ...
              var removeMouseTouchEvents = function () {
                if (_supportPassiveEvents) {
                  ...
                  _hostElement
                    .off(_strMouseTouchMoveEvent, hostOnMouseMove)
            ...
            if (_isTextarea && contentSizeChanged) {
              ...
              if (textareaInfo) {
                ...
                var cursorIsLastPosition =
                  cursorMax === cursorPos && _textareaHasFocus
                var doScroll = {
                  x:
                    !textareaAutoWrapping &&
                    cursorCol === lastCol &&
                ...
                if (doScrollX || doScrollY) {
                  if (doScrollY) _viewportElement[_strScrollTop](doScroll.y)
                  if (doScrollX) {
                    ...
                    else _viewportElement[_strScrollLeft](doScroll.x)
                  }
                }
              }
        ...
        function buildScrollbars() {
          ...
          if (_supportTransition) {
            _scrollbarHorizontalElement.on(
              _strTransitionEndEvent,
              function (event) {
                if (event.target !== _scrollbarHorizontalElement[0]) return
                refreshScrollbarHandleLength(true)
            ...
            _scrollbarVerticalElement.on(
              _strTransitionEndEvent,
              function (event) {
                if (event.target !== _scrollbarVerticalElement[0]) return
                refreshScrollbarHandleLength(false)
                refreshScrollbarHandleOffset(
                  false,
        ...
        function initScrollbarInteractivity(isHorizontal) {
          ...
          function documentMouseTouchUp(event) {
            ...
            _documentElement
              .off(_strMouseTouchMoveEvent, handleDragMove)
          ...
          function onHandleMouseTouchDown(event) {
            ...
            _documentElement
              .on(_strMouseTouchMoveEvent, handleDragMove)
        ...
        function refreshScrollbarHandleOffset(isHorizontal, currentScroll) {
          ...
          if (_supportTransform) {
            transformOffset = isRTLisHorizontal
              ...
              : offset //in px
            transformOffset =
              (transformOffset / trackLength) *
              100 *
              (trackLength / handleLength) //in %
            translateValue = isHorizontal
              ? strTranslateBrace + transformOffset + '%, 0)'
              : strTranslateBrace + '0, ' + transformOffset + '%)'
        ...
        function textareaIsRestrictedKeyCode(keyCode) {
          for (var i = 0; i < _textareaKeyDownRestrictedKeyCodes.length; i++) {
            if (keyCode === _textareaKeyDownRestrictedKeyCodes[i]) return true
          }
          return false
        }
        ...
        _base.destroy = function () {
          ...
          if (_contentGlueElement !== undefined) remove(_contentGlueElement)
          ...
          if (_scrollbarCornerElement) remove(_scrollbarCornerElement)
          if (!_resizeNone) scrollbarCornerOnResized()
          ...
          if (_isTextarea) {
            _targetElement
              .off(_strScroll, textareaOnScroll)
            ...
            if (_msieVersion > 9 || !_autoUpdateRecommended)
              ...
              _targetElement
                .off(_strKeyDownEvent, textareaOnKeyDown)
        ...
        _base.scroll = function (coordinates, duration, easing, complete) {
          ...
          finalScroll[_strScrollLeft] = getFinalScroll(
            true,
            getRawScroll(coordinates).x
          )
          finalScroll[_strScrollTop] = getFinalScroll(
            false,
            getRawScroll(coordinates).y
          )
        ...
        _base.addExt = function (extName, extensionOptions) {
          ...
          if (registeredExtensionObj) {
            if (!_extensions.hasOwnProperty(extName)) {
              instance = registeredExtensionObj.extension.call(
                _base,
        ...
        function construct(targetElement, options, extensions) {
          ...
          if (_isTextarea) {
            ...
            _hostElement
              ...
              .wrapInner(
                generateDiv(
                  _classNameViewportElement + _strSpace + _classNameTextInherit
                )
              )
              .wrapInner(
                generateDiv(
            ...
            _targetElement
              .on(_strScroll, textareaOnScroll)
            ...
            if (_msieVersion > 9 || !_autoUpdateRecommended) {
              ...
              _targetElement
                .on(_strKeyDownEvent, textareaOnKeyDown)
            ...
            _mutationObserverContent = new mutationObserver(
              function (mutations) {
                ...
                if (doUpdate) {
                  ...
                  if (
                    _mutationObserverContentLag <= 0 ||
                    now - contentLastUpdate > _mutationObserverContentLag ||
                    !sizeAuto
                  )
                    action()
                  else
          ...
          if (framework.isArray(extensions))
            framework.each(extensions, function () {
              _base.addExt(this)
            })
      ...
      window[PLUGINNAME] = function (
        ...
        if (framework.isPlainObject(options)) {
          if (pluginTargetElements && pluginTargetElements.length) {
            ...
            result = OverlayScrollbarsInstance(
              pluginTargetElements,
              options,
              extensions,
              _pluginsGlobals,
          ...
          if (pluginTargetElements.length && pluginTargetElements.length > 0) {
            framework.each(pluginTargetElements, function () {
              inst = instances(this)
              if (options === '!') {
                if (inst instanceof window[PLUGINNAME]) arr.push(inst)
              } else arr.push(inst)
            })
            result = arr.length > 1 ? arr : arr[0]
          } else result = instances(pluginTargetElements)
        }
        return result
      }
