          ...
          <ul className="list-group">
            <li className="list-group-item" >
              <a href="#" className="d-flex justify-content-between align-items-center" onClick={this.setWindow('all')} >
                <span className="count badge badge-info" title="Window Id">None</span>
              All Windows
              <span className={`count badge badge-`+(totalTabCount>50 ? `warning` : `success`)} title="Tab Count">{totalTabCount}</span>
              </a>
            </li>
          <li className="list-group-item ">
            <a href="#" className="d-flex justify-content-between align-items-center" onClick={this.setWindow(currentWindowId)}>
            <span className="count badge badge-info" title="Window Id">{currentWindowId}</span>
            Current Window
            <span className={`count badge badge-`+(currentWindowTabs>50 ? `warning` : `success`)} title="Tab Count">{currentWindowTabs}</span>
            </a>
          </li>
          {
            this.state.browserWindows.map(window => {
              console.log("window map", window)
              return (
                <li className="list-group-item" onClick={this.setWindow(window.id)} key={window.id}>
                  <a href="#" className="d-flex justify-content-between align-items-center" onClick={this.setWindow(window.id)}>
                  <span className="count badge badge-info" title="Window Id">{window.id}</span>
                    Window
                  <span className={`count badge badge-`+(window.tabs.length>50 ? `warning` : `success`)} title="Tab Count">{window.tabs.length}</span>
                  </a>
                </li>
            )
            })
